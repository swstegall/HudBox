...
sidebar_position: 1
title: HudBox
description: Lightweight GTK4 heads‑up display (HUD) window for Linux desktops. Load any URL, control opacity and transparency, drag to move, and configure via JSON.
---
...
- Auto‑generates a default config at `~/.hudbox.json` if one is missing
...
HudBox provides a standard CMake install target. You can install it either for your user account (recommended) or system‑wide.
...
- When `transparent` is `true`, the GTK window uses a transparent background and an injected user style sheet forces the page background to transparent. Some websites with aggressive styling might still render opaque elements — that’s content‑specific.
